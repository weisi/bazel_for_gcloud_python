#!/usr/bin/env fish
# Generate the GAE shim and deployment script from Bazel zip output.
...
set -l options \
  ...
  (fish_opt -s a -l descriptor_file    --long-only --required-val) \
...
cp $_flag_descriptor_file $output_real_path/app.yaml

touch $output_real_path/.gcloudignore
